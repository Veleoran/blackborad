...
it('PUT /trips/:departure/:arrival', async () => {
	const newTrip2 = { departure: 'Paris', arrival: 'Montpellier' };

	await request(app).post('/trips').send(newTrip);
	await request(app).put(`/trips/${newTrip.departure}/${newTrip.arrival}`).send(newTrip2);
	const res = await request(app).get('/lastTrip');

	expect(res.statusCode).toBe(200);
	expect(res.body.lastTrip).toEqual(newTrip2);
});
